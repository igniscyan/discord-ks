        ...
        try {
            ...
            collector.on('end', async collected => {
                if (collected.size === 0) {
                    ...
                    db.get(`SELECT * FROM backers WHERE email = ?`, [email], (err, row) => {
                        if (err) {
                            console.error(err.message);
                        }
                        if (row) {
                            ...
                            let rewardTier = row.reward_tier;
                            ...
                            user.send(message);
                            const codeFilter = m => m.author.id === user.id; 
                            const codeCollector = dmChannel.createMessageCollector({ codeFilter, time: 300000 }); // 5 minutes timeout
                            codeCollector.on('collect', async message => {
                                ...
                                if (trimmedCode === redeemToken) {
                                    ...
                                    user.send('Congratulations! You have successfully redeemed your rewards.');
                                }
                                else {
                                    user.send('Sorry, the code you entered is incorrect. Please try again.');
                                
                            }})
                        }
                        else {
                            //user is not in the database
                            user.send('Sorry, we could not find your email in our database.');
                        }
                    });
